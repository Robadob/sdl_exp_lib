...
private:
	...
	*/
	...
	{
		DynamicUniformDetail(const GLenum type, const void* const data, const unsigned count, const char* const uniformName)
			: type(type),
			  data(data),
			  count(count),
	...
	*/
	...
	{
		StaticUniformDetail(const GLenum type, const glm::ivec4& data, const unsigned count, const char* const uniformName)
			: type(type),
			  data(data),
			  count(count),
			  uniformName(uniformName)
	...
	*/
	...
	{
		UniformTextureDetail(const GLuint name, const GLenum type)
			: name(name),
			  type(type)
		{ }
		/**
	...
	*/
	struct BufferDetail
	{
		BufferDetail(const char* const nameInShader, const GLenum type, const GLuint name)
...
//Disable warnings about need to export STL template, these members are private
#pragma warning(push)
